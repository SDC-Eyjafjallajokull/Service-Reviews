const Product = require('./Service-Reviews/database/index.js');
...
const createReview = () => {
  ...
  review.ratings = {
    ...
    graphics: Math.floor(Math.random() * 5),
    lastingQuality: Math.floor(Math.random() * 5),
...
var products = gameNames.map((item) => {
  ...
  for (var key in ratingData) {
    ratingData[key] = ratingData[key] / reviews.length;
  }
